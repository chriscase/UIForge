...
class MarkdownReporter implements Reporter {
  private testResults: Map<string, { passed: number; failed: number; skipped: number; tests: Array<{ name: string; status: string; duration: number }> }> = new Map()
  ...
  onEnd(result: FullResult) {
    ...
    const sortedResults = Array.from(this.testResults.entries()).sort((a, b) => a[0].localeCompare(b[0]))
    ...
    if (totalFailed > 0) {
      ...
      for (const [fileName, fileResult] of sortedResults) {
        const failedTests = fileResult.tests.filter(t => t.status === 'failed' || t.status === 'timedOut')
